...
<div id="lecture" data-status-url="{{ route('lectures.status', $lecture) }}" data-status-route="lectures.status" data-terminal="{{ $terminal ? '1':'0' }}">
  ...
  @else
    {{-- ── Готовый конспект ── --}}
    @php $sj = $lecture->summary_json ?? []; @endphp
    <div class="grid lg:grid-cols-[260px_1fr] gap-6 lg:gap-8 anim-fade-up">
          ...
          <div class="card p-4">
            ...
            <nav class="space-y-0.5 text-sm">
              @foreach ($sj['sections'] as $i => $section)
                <a href="#sec-{{ $i }}" class="block px-2.5 py-1.5 rounded-lg transition-colors line-clamp-1"
                   style="color:var(--ink2)" onmouseover="this.style.background='var(--soft)'" onmouseout="this.style.background='transparent'">
                  {{ $section['heading'] ?? '—' }}
                </a>
              @endforeach
            </nav>
      ...
      {{-- Документ --}}
      <article class="card p-6 sm:p-10 max-w-none">
        ...
        @foreach (($sj['sections'] ?? []) as $i => $section)
          <section id="sec-{{ $i }}" class="scroll-mt-24 {{ $loop->first ? '' : 'mt-10' }}">
            <h2 class="font-serif-display text-2xl mb-3 pb-2" style="border-bottom:1px solid var(--line)">{{ $section['heading'] ?? '' }}</h2>
