...
[ProcessLectureJob] (в очереди)
   ...
   2. TranscriptionService (Groq Whisper) — сегменты последовательно с throttle/
                           backoff (RPM-лимиты Groq) → текст, склейка через пробел
                           (перекрытие ~2с не режет слова; незначительный повтор на
                           стыке сглаживается суммаризатором); transcript_text;
                           status = summarizing
...
- Длинные/тяжёлые файлы: `AudioPreparer` сжимает (16 kHz mono) и режет на сегменты
  под лимит Groq (~20 МБ/сегмент) с перекрытием ~2с, чтобы не резать слова на стыке;
  транскрипты склеиваются через пробел (незначительный повтор на границе допустим).
...
- **D-4. Нарезка аудио по фикс-времени режет слова на стыках.** Решение (MVP):
  нарезка по времени с перекрытием ~2с, чтобы слово не пропало на стыке; небольшой
  повтор на границе допустим и сглаживается суммаризатором. Нарезка по тишине
  (silencedetect) — возможное будущее улучшение.
